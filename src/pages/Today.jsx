...
export default function Today() {
  const { habitsDueToday, completeHabit, errands, completeErrand, tasks, completeTask } = useStore()
  ...
  const hasAnything = habits.length + todayErrands.length + todayTasks.length > 0

  return (
    <Layout title={format(new Date(), 'EEEE, MMM d')}>
      {!hasAnything && (
        <div className="flex flex-col items-center justify-center h-64 gap-3 text-center">
          <span className="text-4xl">✓</span>
          <p className="text-gray-500 dark:text-gray-400 text-sm">You&apos;re all caught up for today.</p>
        </div>
      )}

      {habits.length > 0 && (
        <section className="mt-6">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Habits</h2>
      ...
      {todayErrands.length > 0 && (
        <section className="mt-6">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Errands</h2>
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {todayErrands.map((e) => (
      ...
      {todayTasks.length > 0 && (
        <section className="mt-6">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Tasks</h2>
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {todayTasks.map((t) => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-3 py-3">
                  ...
                  <span className={`text-xs font-medium px-1.5 py-0.5 rounded shrink-0 ${p.color} ${p.bg}`}>{p.label}</span>
                </div>
              )
            })}
          </div>
